...
class GraphXAIExplainer:
    ...
    def generate_risk_explanation(self, politician_id: str, ira_score: float) -> Dict[str, Any]:
        """
        ...
        narrative = self.build_narrative(politician_id, path_analyses, ira_score)
    ...
    def find_contract_paths(self, politician_id: str, max_degrees: int = 3) -> List[List[str]]:
        """
    ...
    def build_narrative(
        self, politician_id: str, path_analyses: List[PathAnalysis], ira_score: float
    ) -> str:
        """
        ...
        for idx, analysis in enumerate(path_analyses[:3], 1):
            ...
            if analysis.temporal_correlation and analysis.temporal_correlation > 0.7:
                narrative += "📅 **Correlación Temporal Significativa:** "
    ...
    def _generate_executive_summary(
        self, path_analyses: List[PathAnalysis], ira_score: float
    ) -> str:
        ...
        summary += "entre el político y contratos públicos, con una separación "
        summary += f"promedio de **{avg_separation:.1f} grados**. "

        if total_amount > 0:
            summary += "Los montos totales involucrados ascienden a "
    ...
    def _extract_key_findings(self, path_analyses: List[PathAnalysis]) -> List[Dict]:
        ...
        highest_value = max(path_analyses, key=lambda p: p.monetary_amount, default=None)
    ...
    def _generate_recommendations(
        ...
        if any(p.ghost_companies > 0 for p in path_analyses):
            recommendations.append("Verificar actividad operativa real de empresas identificadas")
        ...
        recommendations.append("Contrastar hallazgos con expertos en contratación pública")
    ...
    def _calculate_temporal_correlation(self, timeline: List[datetime]) -> float:
        ...
        differences = [
            (sorted_times[i + 1] - sorted_times[i]).days for i in range(len(sorted_times) - 1)
        ]
...
def demonstrate_xai_system():
    ...
    explanation = explainer.generate_risk_explanation(politician_id="POL_001", ira_score=72.5)
