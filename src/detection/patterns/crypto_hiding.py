...
class CryptoHidingDetector:
    ...
    def detect(
        self, politician_id: str, graph_data: Dict, completeness: float = 1.0
    ) -> CryptoHidingResult:
        """
        ...
        mixer_count = sum(w.get("mixer_interactions", 0) for w in crypto_wallets)
        ...
        privacy_coin_holders = [w for w in crypto_wallets if w.get("privacy_coin_holdings", False)]
        ...
        cross_chain_count = sum(w.get("cross_chain_transactions", 0) for w in crypto_wallets)

        if cross_chain_count > 0:
            cross_chain_score = min(self.weights["cross_chain"], cross_chain_count * 2)
        ...
        structured_patterns = [
            w for w in crypto_wallets if w.get("structured_pattern_detected", False)
        ]
