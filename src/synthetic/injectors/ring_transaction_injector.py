...
class RingTransactionInjector:
    ...
    def _generate_ring_network(
        self, politician_id: str, num_intermediaries: int
    ) -> List[Dict[str, Any]]:
        """
    ...
    def inject(self, case: Dict[str, Any]) -> Dict[str, Any]:
        ...
        transactions, intermediaries = self._generate_ring_network(
            politician_id, num_intermediaries
        )
        ...
        case["network_data"]["complex_network_score"] = min(
            1.0, case["network_data"]["complex_network_score"] + 0.3
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Primera Transacción: {first_transaction['source_id']} -> {first_transaction['target_id']} ({first_transaction['amount']})"
    )
